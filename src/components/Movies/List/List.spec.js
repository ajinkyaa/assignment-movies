...
import { setSearchDisplayList } from '../../../redux/features/searchMovieSlice';
...
describe("Movie List component", () => {
    ...
    test("Movie list without data", async () => {
        render(<Provider store={store}><List /></Provider>)
        await act( async () =>{
            store.dispatch(setMovies([]));
        })
        const movieList = screen.getByTestId("no-movie-found");
        expect(movieList).toBeInTheDocument();
    })

    test("Movie list with data", async () => {
        render(<Provider store={store}><List /></Provider>)
        await act( async () =>{
            store.dispatch(setMovies(movies));
        })
        const movieList = screen.getByTestId("movie-list-container");
        expect(movieList).toBeInTheDocument();
    })
})
